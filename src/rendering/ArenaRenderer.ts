...
export class ArenaRenderer {
    ...
    create(): ArenaRendererResult {
        ...
        const canopy = this.scene.add
            .tileSprite(0, 0, width, height * 0.55, spriteCookAssetKey(['players', 'arenas', 'dense-jungle', 'tiles', 'canopy']))
            .setOrigin(0, 0)
            .setScrollFactor(0.3)
            .setAlpha(0.95);

        const mid = this.scene.add
            .tileSprite(0, height * 0.1, width, height * 0.75, spriteCookAssetKey(['players', 'arenas', 'dense-jungle', 'tiles', 'mid']))
            .setOrigin(0, 0)
            .setScrollFactor(0.6)
            .setAlpha(0.96);

        const floor = this.scene.add
            .tileSprite(0, height * 0.52, width, height * 0.48, spriteCookAssetKey(['players', 'arenas', 'dense-jungle', 'tiles', 'floor']))
            .setOrigin(0, 0)
            .setScrollFactor(1)
            .setAlpha(0.98);
